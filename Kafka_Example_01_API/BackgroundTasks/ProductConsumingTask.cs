    ...
    public class ProductConsumingTask : IConsumingTask<string, string>
    {
        ...
        public async Task ExecuteAsync(ConsumeResult<string, string> result)
        {
            ...
            _logger.LogInformation(
                $"[ProductConsumingTask] Consuming message from topic: {result.Topic}, Partition: {result.Partition}, Offset: {result.Offset}, Key: {result.Message.Key}");
            ...
            switch (productEvent)
            {
                case "InsertProduct":
                    ...
                    _logger.LogInformation("[ProductConsumingTask] Inserting new product: {@Product}", product);
                ...
                case "UpdateQuantity":
                    ...
                    _logger.LogInformation("[ProductConsumingTask] Updating quantity for product ID: {ProductId}, New Quantity: {Quantity}, Increase: {Increase}",
                ...
                case "UpdatePrice":
                    ...
                    _logger.LogInformation("[ProductConsumingTask] Updating price for product ID: {ProductId}, New Price: {Price}", p.ProductId, p.Price);
